...
$dbSyncLogin = 'planetsync';
$dbSyncPass = 'pl4n3t0rg4n1c';
$dbSyncHost = 'bbdd4.biomundo';
$dbSyncPort = '3307';
/** **/
...
if ($oDbPla->num_rows($pprs)) {
	...
	while ($aIDP = $oDbPla->fetch_row($pprs)) {
		...
		for ( $idx=$oPPlanet->first_detail(); isset($idx); $idx=$oPPlanet->next_detail()) {
			...
			$txt = $oPPlanet->get_detail_comment($idx);
			$tmpidx = $oPBio->add_detail($idart,$cant);
			if (!empty($txt)) $oPBio->set_detail_comment($tmpidx,$txt);
		}
