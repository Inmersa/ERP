...
foreach ($antiguo as $idx => $clave) {
	...
	$idrs = $db->query("SELECT id_pedido, referencia, fecha FROM Pedidos WHERE id_pedido = $clave");
	if (!$db->num_rows($idrs)) {
		salida("El id Pedido $clave no existe<br/>\n");
		continue;
	}
	$db->libera($idrs);
	...
	$clave_new = $nuevo[$idx];
	if (!isset($clave_new)) {
		salida("($clave_new) Introduicir ref. externa valida para el id Pedido $clave <br/>\n");
		continue;
	}

	$cons = "Update Pedidos set referencia_externa='$clave_new' WHERE id_pedido=$clave";
	$db->query($cons);
	salida("El ID Pedido ID $clave ha pasado a tener una referencia externa: '$clave_new'");
}
