...
function getvar($name){
    ...
    if (isset($_GET[$name])) return $_GET[$name];
			...
			else if (!strcasecmp($name,'mode')) return 'jpg';
			else if (!strcasecmp($name,'encoding')) return '128B';
			else if (!strcasecmp($name,'scale')) return '2';
    else return false;
}
...
barcode_print($code,getvar('encoding'),getvar('scale'),getvar('mode'),getvar('height'));
